...
ui:element({
    id = "control",
    type = "label",
    rect = { unit = "px", x = 12 * S, y = 8 * S, w = W - 24 * S, h = 24 * S },
    props = { text = "Default face, for comparison: abcdefg 0123" },
    style = { font_size = math.floor(16 * S), color = "#94A3B8", align = "left" },
})

-- One row per name, wide enough that a rejected name printing its whole tag does not wrap
local row = math.min(48 * S, (H - 44 * S) / #NAMES)
for i, name in ipairs(NAMES) do
    ui:element({
        id = "name" .. i,
        type = "label",
        rect = { unit = "px", x = 12 * S, y = math.floor(40 * S + (i - 1) * row), w = W - 24 * S, h = math.floor(row) },
        props = { text = '<font="' .. name .. '">' .. name .. ' 0123' },
        style = { font_size = math.floor(math.min(18 * S, row - 8 * S)), color = "#22C55E", align = "left" },
    })
end
